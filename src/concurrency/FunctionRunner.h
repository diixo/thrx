...
#ifndef _CONCURRENCY_FUNCTION_RUNNER_H
#define _CONCURRENCY_FUNCTION_RUNNER_H 1
...
public:
  ...
  void run() override {
    if (repFunc_) {
      while (repFunc_()) {
        THRX_SLEEP_USEC(intervalMs_ * 1000);
      }
    } else {
      func_();
    }
...
#endif // #ifndef _CONCURRENCY_FUNCTION_RUNNER_H
